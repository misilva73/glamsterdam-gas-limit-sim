# Fees and block utilisation in the first day after Glamsterdam

#### Maria Silva, September 2026

What happens to the fee market when Glamsterdam changes transaction gas costs and the gas limit rises from 60M to 200M? More capacity should ease pressure on fees, but cheaper gas also attracts demand. The outcome depends on how much demand arrives and how strongly users respond to the change in price.
...
Under the demand assumptions tested here, the day has a clear sequence: an **initial fee spike, a decline as capacity expands and demand responds, and then adjustment around the new 100M target**. The size of the spike depends heavily on how the simulation introduces demand.
...
1. **Sample arrivals.** A demand model determines how much transaction gas arrives: higher prices reduce demand, and lower prices increase it. We sample historical transactions to meet that quantity and add them to the mempool alongside transactions waiting from earlier blocks.
...
Finally, block capacity follows two new accounting rules:

1. Execution gas and state-creation gas must each fit within the limit, and utilisation is the **larger of the two divided by the limit**.
2. Capacity is measured before refunds.
...
We simulate 50 paths for each of the twelve combinations. The utilisation and base-fee charts show the initial 3,000 blocks, or roughly ten hours, covering the ramp and the early settling period. The demand-response chart then shows the full 7,200-block day. Lines are means across paths and shaded bands show their 10th–90th percentile; the full-day chart uses 24-block averages within each path. On the horizontal axis, 1,200 blocks is about four hours.

## The first several hours: fees rise before capacity catches up

The transition starts with demand arriving against a 60M limit. The higher-demand scenarios initially fill blocks close to that limit, while even the 1x scenario starts above the 30M fee target. The following plot shows this trend. The horizontal dashed line marks the 50% utilisation target, while the vertical dashed line marks the end of the ramp at block 1,234.

![Block utilization over the first 3,000 simulated blocks](./figures/gas_limit_simulation/block_utilization.png)
...
![Base fee over the first 3,000 simulated blocks](./figures/gas_limit_simulation/base_fee.png)
...
Most adjustment occurs in the first several hours, with weaker price responses taking longer to recover. Fees and utilisation have approximately stabilised in all scenarios by block 3,000, about ten hours into the simulation. We use blocks **3,000–7,199**, the remaining fourteen hours, for all settled-period medians below. Comparing the two halves of this window using 24-block averages, median utilisation changes by less than 0.09 percentage points in every scenario; median base fees change by at most 4.24%, with a largest absolute change of 0.0071 gwei.

## The rest of the day: two outcomes emerge

Once the limit reaches 200M, the scenarios separate into two groups: those that attract enough demand to approach the 100M fee target, and those that remain below it even as the base fee becomes negligible.

Under this transaction mix, reaching the 100M fee target requires roughly **2.54 times the reference demand quantity**. The horizontal dashed line in the plot below marks that multiplier; the vertical dashed line marks the end of the ramp at block 1,234. The plot follows demand through the full day, from its initial fall to its recovery as prices ease.

![Demand multiplier over the full 7,200 simulated blocks](./figures/gas_limit_simulation/appendix_full_window_demand_response.png)
...
Reaching the same block utilisation does not mean paying the same price. Among the nine scenarios near 50% utilisation, different demand levels and price responses sustain very different base fees.

The chart below compares median base fees over blocks 3,000–7,199, after fees and utilisation have approximately stabilised in all scenarios. The dashed line marks the starting fee of about 0.085 gwei.

<img src="./figures/gas_limit_simulation/clearing_price_by_demand.png" alt="Settled-period median base fee by demand level and price elasticity" width="70%">
...
At **2.5x demand**, the scenarios already start close to the quantity needed at 200M, so less price adjustment is required. Their settled-period median fees are closer together, around **0.32–0.39 gwei**. That narrowing does not mean elasticity is unimportant: it has a large effect on the early fee spike and on the lower-demand outcomes. The grid ends at 2.5x, so it cannot establish what happens at higher demand levels.

For context, the simulation starts at a base fee of **0.0847 gwei**, taken from the first source block. Eight of the twelve settled-period medians are below that starting value. The 2x scenario with elasticity 0.3 and all three 2.5x scenarios are above it. A larger limit therefore does not guarantee a lower base fee if enough demand arrives.
...
The price-response factor is bounded between 0.05 and 20 before multiplication by the demand level. Complete configuration, accounting rules and output definitions are in the [methodology](../METHODOLOGY.md), with analysis and supporting diagnostics in the [notebook](../analysis.ipynb).
